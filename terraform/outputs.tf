...
[aws]
${aws_instance.jenkins_server.public_ip} ansible_user=ec2-user ansible_ssh_private_key_file=~/.ssh/${var.key_name}.pem
EOF
    EOT
  }
...
output "ssh_command" {
  ...
  value       = "ssh -i ~/.ssh/${var.key_name}.pem ec2-user@${aws_instance.jenkins_server.public_ip}"
}
